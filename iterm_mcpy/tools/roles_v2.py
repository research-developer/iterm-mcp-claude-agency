"""SP2 method-semantic `roles_v2` tool — Task 11.

Eighth SP2 collection tool (after sessions_v2 + agents_v2 + teams_v2 +
managers_v2 + feedback_v2 + memory_v2 + services_v2). Replaces the legacy
``list_available_roles`` and ``check_tool_permission`` tools.

Roles are a read-only catalog in SP2 — role *assignment* happens through
``sessions_v2`` (target='roles'), which already exposes PATCH/DELETE for
...
under ``roles_v2`` to preserve legacy semantics — a future refactor may
move it into ``sessions_v2``.

Registered under the provisional name ``roles_v2`` to coexist with the
...
class RolesDispatcher(MethodDispatcher):
    ...
    assignment to sessions happens through sessions_v2 (target='roles').
    """
...
async def roles_v2(
    ...
    by ``sessions_v2`` (target='roles'), not here — roles_v2 is read-only.
    """
    raw_params = {
        "target": target,
        "session_id": session_id,
...
def register(mcp):
    """Register the roles_v2 dispatcher tool.

    Named ``roles_v2`` to coexist with the legacy ``list_available_roles``
    ...
    """
    mcp.tool(name="roles_v2")(roles_v2)
