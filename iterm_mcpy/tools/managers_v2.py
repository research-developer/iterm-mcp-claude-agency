"""SP2 method-semantic `managers_v2` tool — Task 7.

Fourth SP2 collection tool (after sessions_v2 + agents_v2 + teams_v2).
...
Registered under the provisional name ``managers_v2`` to coexist with the
...
class ManagersDispatcher(MethodDispatcher):
    ...
    async def _list_managers(self, ctx):
        ...
        logger.info(f"managers_v2 GET: listed {len(result)} managers")
    ...
    async def _get_manager_info(self, ctx, manager_name: str):
        ...
        logger.info(f"managers_v2 GET: info for manager '{manager_name}'")
        return {
            "name": manager.name,
            "workers": manager.workers,
    ...
    async def _create_manager(self, ctx, **params):
        ...
        # always is — but keeping parity with teams_v2 style keeps this
        # predictable).
        ...
        logger.info(
            f"managers_v2 CREATE: created manager '{manager.name}' with "
    ...
    async def _add_worker(self, ctx, **params):
        ...
        logger.info(
            f"managers_v2 CREATE workers: added worker '{worker_name}' to "
    ...
    async def _remove_manager(self, ctx, **params):
        ...
        logger.info(f"managers_v2 DELETE: removed manager '{manager_name}'")
    ...
    async def _remove_worker(self, ctx, **params):
        ...
        logger.info(
            f"managers_v2 DELETE workers: removed worker '{worker_name}' "
...
def register(mcp):
    """Register the managers_v2 dispatcher tool.

    Named ``managers_v2`` to coexist with the legacy ``manage_managers``
    tool during the SP2 coexistence period. Final cutover (renaming to
    ``managers``) happens at the end of SP2.
    """
    mcp.tool(name="managers_v2")(managers_v2)
