...
class SessionsDispatcher(MethodDispatcher):
    ...
    async def _create_sessions(self, ctx, **params):
        ...
        logger.info(f"sessions CREATE: created {len(result.sessions)} sessions")
        return result
    ...
    async def _patch_session(self, ctx, definer, **params):
        ...
        # (sessions is session-centric; the modification helper itself
...
def register(mcp):
    """Register the sessions dispatcher tool."""
    mcp.tool(name="sessions")(sessions)
