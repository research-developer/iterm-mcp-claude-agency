"""SP2 method-semantic `workflows_v2` tool — Task 12.
...
Registered under the provisional name ``workflows_v2`` to coexist with the
...
# Priority level mapping — copied from the legacy tool so workflows_v2 can
...
class WorkflowsDispatcher(MethodDispatcher):
    ...
    async def _list_events(self, ctx, **params):
        ...
        if logger is not None:
            logger.info(f"workflows_v2 GET events: listed {len(events)} events")
    ...
    async def _history(self, ctx, **params):
        ...
        if logger is not None:
            logger.info(
                f"workflows_v2 GET history: {len(entries)} entries "
    ...
    async def _trigger(self, ctx, **params):
        ...
        if logger is not None:
            logger.info(
                f"workflows_v2 TRIGGER: event={event_name!r} "
...
def register(mcp):
    """Register the workflows_v2 dispatcher tool.

    Named ``workflows_v2`` to coexist with the legacy
    ``trigger_workflow_event``, ``list_workflow_events``, and
    ...
    """
    mcp.tool(name="workflows_v2")(workflows_v2)
