"""SP2 method-semantic `services_v2` tool — Task 10.

Seventh SP2 collection tool (after sessions_v2 + agents_v2 + teams_v2 +
managers_v2 + feedback_v2 + memory_v2). Replaces the legacy
...
Registered under the provisional name ``services_v2`` to coexist with the
...
class ServicesDispatcher(MethodDispatcher):
    ...
    async def _list(self, ctx, **params):
        ...
        logger.info(f"services_v2 GET: listed {len(result)} services")
        return {
            "services": result,
            "count": len(result),
            "repo_path": repo_path,
    ...
    async def _list_inactive(self, ctx, **params):
        ...
        logger.info(
            f"services_v2 GET inactive: {len(result)} inactive in {repo_path}"
        )
        return {
    ...
    async def _add(self, ctx, **params):
        ...
        logger.info(
            f"services_v2 CREATE: added service '{service_name}' to {scope} config"
        )
        return {
    ...
    async def _start(self, ctx, **params):
        ...
        logger.info(
            f"services_v2 TRIGGER: start '{service_name}' -> "
    ...
    async def _configure(self, ctx, **params):
        ...
        logger.info(
            f"services_v2 MODIFY: updated service '{service_name}' in {scope} config"
        )
        return {
            "service": service_name,
            "scope": scope,
    ...
    async def _stop(self, ctx, **params):
        ...
        logger.info(
            f"services_v2 DELETE: stop '{service_name}' -> success={success}"
        )
        return {
            "service": service_name,
            "stopped": success,
...
def register(mcp):
    """Register the services_v2 dispatcher tool.

    Named ``services_v2`` to coexist with the legacy ``manage_services``
    tool during the SP2 coexistence period. Final cutover (renaming to
    ``services``) happens at the end of SP2.
    """
    mcp.tool(name="services_v2")(services_v2)
