"""SP2 method-semantic `memory_v2` tool — Task 9.

Sixth SP2 collection tool (after sessions_v2 + agents_v2 + teams_v2 +
managers_v2 + feedback_v2). Replaces the legacy ``manage_memory`` tool's
8 operations:
...
Registered under the provisional name ``memory_v2`` to coexist with the
...
class MemoryDispatcher(MethodDispatcher):
    ...
    async def _retrieve(self, ctx, **params):
        ...
        if memory:
            logger.info(
                f"memory_v2 GET: retrieved {'/'.join(namespace)}/{key}"
            )
            return {
                "found": True,
                "key": memory.key,
        ...
        logger.info(
            f"memory_v2 GET: not found {'/'.join(namespace)}/{key}"
        )
    ...
    async def _search(self, ctx, **params):
        ...
        logger.info(
            f"memory_v2 GET search: '{query}' in "
    ...
    async def _list_keys(self, ctx, **params):
        ...
        logger.info(
            f"memory_v2 GET keys: {len(keys)} in {'/'.join(namespace)}"
        )
    ...
    async def _list_namespaces(self, ctx, **params):
        ...
        logger.info(
            f"memory_v2 GET namespaces: {len(namespaces)} listed"
        )
        return {
            "prefix": namespace,
    ...
    async def _stats(self, ctx, **params):
        ...
        logger.info("memory_v2 GET stats: returning stats")
        return stats
    ...
    async def _store(self, ctx, **params):
        ...
        logger.info(f"memory_v2 CREATE: stored {'/'.join(namespace)}/{key}")
        return {
            "status": "stored",
            "namespace": namespace,
            "key": key,
    ...
    async def _delete_key(self, ctx, **params):
        ...
        if deleted:
            logger.info(
                f"memory_v2 DELETE: removed {'/'.join(namespace)}/{key}"
            )
        else:
            logger.info(
                f"memory_v2 DELETE: not found {'/'.join(namespace)}/{key}"
            )
        return {
            "deleted": deleted,
            "namespace": namespace,
    ...
    async def _clear_namespace(self, ctx, **params):
        ...
        logger.info(
            f"memory_v2 DELETE namespace: cleared "
...
def register(mcp):
    """Register the memory_v2 dispatcher tool.

    Named ``memory_v2`` to coexist with the legacy ``manage_memory`` tool
    during the SP2 coexistence period. Final cutover (renaming to
    ``memory``) happens at the end of SP2.
    """
    mcp.tool(name="memory_v2")(memory_v2)
