...
async def main(connection):
    ...
    # Query for the list of profiles so we can search by name. This returns a subset of the full profiles so it's fast.
    ...
    for partial in partialProfiles:
        if partial.name == "Default":
            # This is the one we're looking for. Change the current session's profile.
