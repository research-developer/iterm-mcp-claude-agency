...
Both Claude Code and Claude Desktop spawn the stdio shim (`iterm-mcp` / `python -m iterm_mcpy`). The shim discovers or auto-starts a shared singleton daemon over streamable HTTP on `127.0.0.1:12340-12349`; all clients share one iTerm2 connection and one agent/team registry (cross-client agent visibility). The daemon auto-starts on first client connect; its state is advertised in `~/.iterm-mcp/daemon.json`. The `/health` endpoint serves a version handshake and shims will restart version-stale daemons automatically. The reported version is **auto-derived** — `major.minor` (from package metadata) plus the git commit count as the patch digit (e.g. `0.1.263`) — and is frozen per process via `lru_cache`, so a daemon keeps reporting the code it started with while a freshly spawned shim computes the current checkout. Any new commit therefore changes the version and triggers an automatic restart of the stale daemon, with no manual version bump. Auto-restart fires only on a *confident* signal — both the shim and the daemon must have derived their version from git (`/health` reports a `version_source` of `"git"` vs `"metadata"`); if either environment lacks git (e.g. Claude Desktop launched from Finder with a minimal PATH), a version mismatch is treated as untrustworthy and the shared singleton daemon is left running rather than thrashed. (Uncommitted working-tree edits don't change the commit count, so during local development restart the daemon manually — `iterm-mcp stop` then reconnect.)
